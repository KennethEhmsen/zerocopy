...
def build():
    ...
    # Copies compiled *.pyd files in ./zerocopy directory in order to
    # allow "import zerocopy" when using the interactive interpreter
    # from within this directory.
    sh("%s setup.py build_ext -i" % PYTHON)
    # Make sure it actually worked.
    sh('%s -c "import zerocopy"' % PYTHON)
...
def uninstall():
    """Uninstall zerocopy"""
    # Uninstalling zerocopy on Windows seems to be tricky.
    # On "import zerocopy" tests may import a zerocopy version living in
    # C:\PythonXY\Lib\site-packages which is not what we want, so
    # we try both "pip uninstall zerocopy" and manually remove stuff
    # from site-packages.
    clean()
    install_pip()
    here = os.getcwd()
    try:
        ...
        while True:
            try:
                import zerocopy  # NOQA
            except ImportError:
                break
            else:
                sh("%s -m pip uninstall -y zerocopy" % PYTHON)
    finally:
        os.chdir(here)

    for dir in site.getsitepackages():
        for name in os.listdir(dir):
            if name.startswith('zerocopy'):
